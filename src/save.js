(function () {
  ...
  let saveTimer = null;
  const sanitizeNum = (n, d = 0) => Number.isFinite(n) ? n : d;
  ...
  function migrateSave(raw) {
    ...
    while (v < GameState.SAVE_VERSION) {
      if (v === 0) {
        ...
        save.saveVersion = 2;
        save.units = Array.isArray(save.units) ? Object.fromEntries(save.units.map((u) => [u.id, u.count])) : (save.units || {});
      } else if (v === 2) {
        save.saveVersion = 3;
        save.quests = save.quests?.progress ? save.quests : { progress: save.quests || {}, completed: {}, claimed: {}, cycle: {}, cycleTarget: {} };
      } else if (v === 3) {
        save.saveVersion = 4;
        save.quests = { progress: {}, completed: {}, claimed: {}, cycle: {}, cycleTarget: {}, ...(save.quests || {}) };
        save.units = save.units || {};
        (save.deck || []).forEach((id) => {
          if (save.units[id] && save.units[id] > 0) save.units[id] -= 1;
        });
        Object.keys(save.units).forEach((id) => { if (save.units[id] <= 0) delete save.units[id]; });
      }
      v = save.saveVersion;
    }
    return save;
  ...
  function validate(save) {
    ...
    out.units = out.units || {}; out.inventory = out.inventory || {}; out.buildings = out.buildings || {};
  ...
  function saveNow() { const p = GameState.get(); p.timers.lastSave = Date.now(); localStorage.setItem(KEY, JSON.stringify(p)); }
  function scheduleSave() { if (saveTimer) clearTimeout(saveTimer); saveTimer = setTimeout(saveNow, 600); }

  function load() {
    try {
      const raw = localStorage.getItem(KEY);
      if (!raw) return false;
      GameState.set(validate(migrateSave(JSON.parse(raw))));
      return true;
