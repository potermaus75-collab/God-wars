(function () {
  ...
  function bossElementFromId(id) {
    if (id.includes('_gr_')) return 'fire';
    if (id.includes('_kr_')) return 'earth';
    if (id.includes('_nr_')) return 'water';
    return 'wind';
  }
  ...
  function simulateBossBattle(player, bossId) {
    ...
    for (let turn = 1; turn <= maxTurns; turn += 1) {
      ...
      if (bossHp <= 0) { frames.push({ turn, bossHp, teamHp, logs: logs.slice(-2) }); break; }
      ...
      if (effects.team.stun > 0) {
        ...
        const em = elementMultiplier(unit.element, bossElement);
      ...
      frames.push({ turn, bossHp, teamHp, logs: logs.slice(-2) });
      if (teamHp <= 0) break;
    }

    const win = bossHp <= 0 && teamHp > 0;
    const timeout = !win && teamHp > 0;
    const rewards = win ? { exp: boss.rew_exp, gold: boss.rew_gold, card: boss.drop_card, extraDrops: rollDrops(boss) } : null;
    return { ok: true, win, timeout, logs: logs.slice(-24), frames, rewards, bossHp: Math.max(0, bossHp), teamHp: Math.max(0, teamHp), startTeamHp, startBossHp: boss.hp_max };
  }
